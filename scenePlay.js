var scenePlay = new Phaser.Class({
    ...
    create: function () {
        ...
        // --- TAMBAHAN: Teks "ANDA BERHASIL" ---
        this.successText = this.add.text(X_POSITION.CENTER, Y_POSITION.CENTER, 'ANDA BERHASIL', {
            fontFamily: 'Verdana, Arial',
            fontSize: '60px',
        ...
        this.successText.setDepth(11); // Pastikan di atas layer lain
        this.successText.setVisible(false); // Awalnya sembunyikan
        // --- AKHIR TAMBAHAN ---
        ...
        var collectCoin = function(player, coin)
        {
            ...
            { 
                // --- MODIFIKASI: Cek apakah level 5 sudah selesai ---
                if (currentLevel === 5) { // Jika sudah di level 5 dan semua koin terkumpul
                    activeScene.gameStarted = false;
                    activeScene.physics.pause();
                    activeScene.player.setVelocityX(0); // Hentikan gerakan player
                    activeScene.player.anims.stop(); // Hentikan animasi player
                    activeScene.snd_walk.setVolume(0); // Hentikan suara jalan
                    activeScene.music_play.stop(); // Hentikan musik game

                    activeScene.tweens.add({
                        targets: darkenLayer,
                        duration: 250,
                        alpha : 1, // Tampilkan layar gelap penuh
                        onComplete: function(){
                            activeScene.successText.setVisible(true); // Tampilkan teks "ANDA BERHASIL"
                            // Tambahkan delay sebelum kembali ke menu atau aksi lain jika diperlukan
                            activeScene.time.delayedCall(3000, () => {
                                currentLevel = 1; // Reset level untuk game berikutnya
                                activeScene.scene.start('sceneMenu'); // Kembali ke sceneMenu
                            }, [], activeScene);
                        }
                    });
                    return; // Keluar dari fungsi agar tidak lanjut ke level berikutnya
                }
                // --- AKHIR MODIFIKASI ---
                ...
                activeScene.player.anims.play('front'); // Mengganti 'turn' menjadi 'front' karena 'turn' tidak didefinisikan
        ...
        var prepareWorld = function() {
            ...
            else if (currentLevel == 3) // Menambahkan kondisi spesifik untuk level 3
            {
            ...
            else if (currentLevel == 4) { // Contoh untuk level 4
                platforms.create(200 + relativeSize.w, 300, 'ground');
                platforms.create(500 + relativeSize.w, 200, 'ground');
                platforms.create(800 + relativeSize.w, 350, 'ground');
                platforms.create(100 + relativeSize.w, 570, 'ground');
                platforms.create(700 + relativeSize.w, 570, 'ground');
            }
            else if (currentLevel == 5) { // Contoh untuk level 5
                platforms.create(Phaser.Math.Between(100, game.canvas.width - 100) + relativeSize.w, Phaser.Math.Between(200, 150), 'ground');
                platforms.create(Phaser.Math.Between(100, game.canvas.width - 100) + relativeSize.w, Phaser.Math.Between(400, 400), 'ground');
                platforms.create(Phaser.Math.Between(100, game.canvas.width - 100) + relativeSize.w, Phaser.Math.Between(250, 450), 'ground');
            }
            
            // melakukan pengecekan terhadap level yang
            // sedang aktif, jika level lebih dari 3, maka
